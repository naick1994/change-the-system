// Same drifting "star field" pattern used across the sibling sites
// (ChangeTheTide.tsx / WooCpo.tsx), recolored to this project's orange.
// Fixed behind the whole page, decorative only.
const DOTS = [
  ...
  return (
    <>
      <style>{`
        ...
        .cts-data-dot { animation-name: ctsDataDrift; animation-timing-function: linear; animation-iteration-count: infinite; }
        @media (prefers-reduced-motion: reduce) {
          .cts-data-dot { animation: none; opacity: 0.25; }
        }
      `}</style>
      <div className="fixed inset-0 pointer-events-none overflow-hidden" style={{ zIndex: 0 }} aria-hidden="true">
        {DOTS.map((d, i) => (
          <span
            key={i}
            className="cts-data-dot absolute rounded-full"
            style={{
              left: `${d.x}%`, top: `${d.y}%`,
              width: d.size, height: d.size,
              background: '#f2661a',
              boxShadow: '0 0 5px 1px rgba(242,102,26,0.5)',
